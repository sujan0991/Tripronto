...
@interface KDCycleSplitBannerView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation KDCycleSplitBannerView
...
//        if ((self.isContinuous && _datasourceImages.count > 2) || (!self.isContinuous &&_datasourceImages.count > 1)) {
//            [self performSelector:@selector(autoSwitchBannerView) withObject:nil afterDelay:0];
//        }
//    }
}
...
- (void)initializePageControl {
    CGRect pageControlFrame = CGRectMake(0, 0, CGRectGetWidth(_scrollView.frame), 0);
    _pageControl = [[UIPageControl alloc] initWithFrame:pageControlFrame];
    _pageControl.center = CGPointMake(CGRectGetWidth(_scrollView.frame)*0.9, 10);
...
- (void)loadData {
    ...
    _datasourceImages = [_datasource numberOfKDCycleSplitBannerView:self];
    ...
    CGFloat contentWidth = CGRectGetWidth(_scrollView.frame)/2;
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++) {
      ...
      //   NSLog(@"imgRect =%lf with %i",imgRect.size.width,i);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    CGFloat targetX = scrollView.contentOffset.x;
     CGFloat item_width = CGRectGetWidth(scrollView.frame)/2;
