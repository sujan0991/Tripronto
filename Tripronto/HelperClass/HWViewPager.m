...
-(void)layoutSubviews{
    ...
    if(!CGRectEqualToRect(self.frame, self.beforeFrame)){
        ...
        NSLog(@"isSingle %i" , (int)self.isSingle );
        
        if (self.isSingle) {
            self.flowLayout.itemSize = CGSizeMake(widthNew, heightNew);
        }
        else
            self.flowLayout.itemSize = CGSizeMake(widthNew/2, heightNew);
...
//        NSLog(@"벨로서티가 -기준보다 작아서, 왼쪽으로 이동해야한다!");
    ...
    if(self.userPagerDelegate){
        [self.userPagerDelegate pagerDidSelectedPage:self.selectedPageNum with:self.tag];
    }
