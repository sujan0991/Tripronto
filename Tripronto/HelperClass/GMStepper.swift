...
//@IBDesignable
open class GMStepper: UIControl {
    ...
    /// Text color of the buttons. Defaults to white.
     open var buttonsTextColor: UIColor = UIColor.white {
        didSet {
    ...
    /// Background color of the buttons. Defaults to dark blue.
     open var buttonsBackgroundColor: UIColor = UIColor(red:248/255, green:248/255, blue:248/255, alpha:1) {
        didSet {
            for button in [leftButton, rightButton] {
    ...
    open var buttonsFont = UIFont(name: "AzoSans-Regular", size: 18.0)! {
        didSet {
            for button in [leftButton, rightButton] {
    ...
    /// Text color of the middle label. Defaults to white.
     open var labelTextColor: UIColor = UIColor(red:224/255, green:51/255, blue:101/255, alpha:1){
        didSet {
    ...
    /// Text color of the middle label. Defaults to lighter blue.
     open var labelBackgroundColor: UIColor = UIColor(red:248/255, green:248/255, blue:248/255, alpha:1) {
        didSet {
    ...
    open var labelFont = UIFont(name: "AzoSans-Regular", size: 20.0)! {
        didSet {
    ...
    /// Percentage of the middle label's width. Must be between 0 and 1. Defaults to 0.5. Be sure that it is wide enough to show the value.
     open var labelWidthWeight: CGFloat = 0.5 {
        didSet {
    ...
    /// Color of the flashing animation on the buttons in case the value hit the limit.
     open var limitHitAnimationColor: UIColor = UIColor (red:248/255, green:248/255, blue:248/255, alpha:1)
    ...
    */
    ...
    lazy var leftButton: UIButton = {
        ...
        button.setImage(UIImage(named: "minusStepper"), for: UIControlState())
        
        
        button.titleLabel?.font = self.buttonsFont
        button.addTarget(self, action: #selector(GMStepper.leftButtonTouchDown(_:)), for: .touchDown)
        button.addTarget(self, action: #selector(GMStepper.buttonTouchUp(_:)), for: UIControlEvents.touchUpInside)
        button.addTarget(self, action: #selector(GMStepper.buttonTouchUp(_:)), for: UIControlEvents.touchUpOutside)
        return button
    }()
    ...
    lazy var rightButton: UIButton = {
        ...
        button.setImage(UIImage(named: "plusStepper"), for: UIControlState())
        button.backgroundColor = self.buttonsBackgroundColor
        button.titleLabel?.font = self.buttonsFont
        button.addTarget(self, action: #selector(GMStepper.rightButtonTouchDown(_:)), for: .touchDown)
        button.addTarget(self, action: #selector(GMStepper.buttonTouchUp(_:)), for: UIControlEvents.touchUpInside)
        button.addTarget(self, action: #selector(GMStepper.buttonTouchUp(_:)), for: UIControlEvents.touchUpOutside)
        return button
    }()
...
//        label.addGestureRecognizer(panRecognizer)
    ...
    lazy var rightSeperatorView: UIImageView={
        
         let seperatorView = UIImageView(image: UIImage(named: "stepperSeperator"))
        return seperatorView
    }()
    
    lazy var leftSeperatorView: UIImageView={
...
//                }
    ...
    */
    ...
    open override func layoutSubviews() {
        ...
        leftButton.frame = CGRect(x: 0, y: 0, width: buttonWidth-1, height: bounds.size.height)
        label.frame = CGRect(x: buttonWidth+1, y: 0, width: labelWidth-2, height: bounds.size.height)
        rightButton.frame = CGRect(x: labelWidth + buttonWidth+1, y: 0, width: buttonWidth-1, height: bounds.size.height)
        ...
        leftSeperatorView.frame = CGRect(x: buttonWidth-1, y: 0, width: 2, height: bounds.size.height)
        rightSeperatorView.frame = CGRect(x: labelWidth + buttonWidth-1, y: 0, width: 2, height: bounds.size.height)
...
extension GMStepper {
    ...
    func scheduleTimer() {
        timer = Timer.scheduledTimer(timeInterval: timerInterval, target: self, selector: #selector(GMStepper.handleTimerFire(_:)), userInfo: nil, repeats: true)
    }
